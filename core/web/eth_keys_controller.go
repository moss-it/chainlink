...
func (ekc *ETHKeysController) Index(c *gin.Context) {
	...
	keys, err := store.KeyStore.UnlockedKeys()
	if err != nil {
...
func (ekc *ETHKeysController) Create(c *gin.Context) {
	account, err := ekc.App.GetStore().KeyStore.CreateNewAccount()
	if err != nil {
		jsonAPIError(c, http.StatusInternalServerError, err)
		return
	}
	key, err := ekc.App.GetStore().KeyStore.KeyByAddress(account.Address)
	if err != nil {
		jsonAPIError(c, http.StatusInternalServerError, err)
		return
	}
...
func (ekc *ETHKeysController) Delete(c *gin.Context) {
	...
	if exists := ekc.App.GetStore().KeyStore.HasAccountWithAddress(address); !exists {
		jsonAPIError(c, http.StatusNotFound, errors.New("Key does not exist"))
		return
	}

	key, err := ekc.App.GetStore().KeyStore.KeyByAddress(address)
	if err != nil {
		jsonAPIError(c, http.StatusInternalServerError, err)
		return
	}
	...
	if hardDelete {
		err = ekc.App.GetStore().KeyStore.DeleteKey(address)
	} else {
		err = ekc.App.GetStore().KeyStore.ArchiveKey(address)
	}
	if err != nil {
		jsonAPIError(c, http.StatusInternalServerError, err)
		return
...
func (ekc *ETHKeysController) Import(c *gin.Context) {
	...
	acct, err := store.KeyStore.ImportKey(bytes, oldPassword)
	if err != nil {
		jsonAPIError(c, http.StatusInternalServerError, err)
		return
	}

	key, err := store.KeyStore.KeyByAddress(acct.Address)
	if err != nil {
