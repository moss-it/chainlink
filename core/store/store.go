...
type KeyStoreGenerator func(*gorm.DB, *orm.Config) *KeyStore

func StandardKeyStoreGen(db *gorm.DB, config *orm.Config) *KeyStore {
	scryptParams := utils.GetScryptParams(config)
	return NewKeyStore(db, config.KeysDir(), scryptParams)
}

func InsecureKeyStoreGen(db *gorm.DB, config *orm.Config) *KeyStore {
	return NewInsecureKeyStore(db, config.KeysDir())
}
...
func newStoreWithKeyStore(
	...
	// This must be done before initializing the keystore to ensure all DB keys
	// get picked up by geth key store (which reads from filesystem)
	// FIXME: Can we improve this, seems kinda hacky to go via filesystem?
...
func (s *Store) Start() error {
	return s.KeyStore.SyncDiskKeyStoreToDB()
}
